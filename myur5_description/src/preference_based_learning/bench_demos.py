...
def batch(task, method, N, M, b):
    ...
    for j in range(b):
        ...
        psi, s = bench_get_feedback(simulation_object, input_A, input_B, true_w)
        psi_set.append(psi)
        s_set.append(s)
    i = b
    m = 0
    ...
    while i < N:
        ...
        for j in range(b):
            input_A = inputA_set[j]
            input_B = inputB_set[j]
            psi, s = bench_get_feedback(simulation_object, input_B, input_A, true_w)
            psi_set.append(psi)
            s_set.append(s)
    ...
    plt.ylabel('m')
    plt.xlabel('N')
    plt.savefig('./outputs/bench_output.png')
    plt.show()
